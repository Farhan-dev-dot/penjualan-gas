...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const hasUploadedKtp = config.hasUploadedKtp;
    ...
    function renderCart() {
        ...
        document
            .querySelectorAll(".cart-count-badge, .cart-nav-badge")
            .forEach((element) => {
        ...
        if (!items.length) {
            cartBody.innerHTML = `
                <div class="cart-empty flex-grow-1 d-flex flex-column align-items-center justify-content-center text-center">
                    ...
                    <a href="${config.produkUrl}" class="btn btn-cart-primary px-4">Lihat Produk</a>
                </div>`;
            return;
        }
        ...
        cartBody.innerHTML = `
            ...
            <div class="cart-summary">
                ...
                <a href="${hasUploadedKtp ? config.checkoutUrl : profileUrl}" class="btn btn-cart-primary w-100 mt-3">${hasUploadedKtp ? "Checkout Sekarang" : "Unggah KTP untuk Checkout"} <i class="fa-solid fa-arrow-right ms-2"></i></a>
            </div>`;
    }
    ...
    document.addEventListener("click", function (event) {
        ...
        const actionButton = event.target.closest(
            ".cart-plus, .cart-minus, .cart-item-remove",
        );
        ...
        if (addButton && !addButton.disabled) {
            ...
            if (!hasUploadedKtp) {
                alert(
                    "Tolong lengkapi profil dan unggah foto KTP terlebih dahulu.",
                );
            ...
            cart[product.productId].quantity = Math.min(
        ...
        if (actionButton) {
            ...
            if (actionButton.classList.contains("cart-minus"))
                item.quantity = Math.max(item.quantity - 1, 1);
            if (actionButton.classList.contains("cart-item-remove"))
                delete cart[key];
            saveCart(cart);
        }
    });
